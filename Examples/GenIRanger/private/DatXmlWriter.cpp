...
{
  mXml << std::string(mIndent, ' ');
  mXml << "<component valuetype=\"" << valueType << "\" name=\"" << name
       << "\">" << std::endl;
  indent();
}
...
void DatXmlWriter::openWorldRangeTraits()
{
	mXml << std::string(mIndent, ' ');
	mXml << "<worldrangetraits>" << std::endl;
	indent();
}
...
void DatXmlWriter::closeWorldRangeTraits()
{
	unIndent();
	mXml << std::string(mIndent, ' ');
	mXml << "</worldrangetraits>" << std::endl;
...
{
  mXml << std::string(mIndent, ' ');
  mXml << "<subcomponent valuetype=\"" << valueType << "\" name=\"" << name
       << "\">" << std::endl;
  indent();
}
...
void DatXmlWriter::addArbitraryXml(std::string arbitraryXml)
{
  if (!arbitraryXml.empty())
  {
    mXml << arbitraryXml << std::endl;
  }
}
